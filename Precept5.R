...
cmat <- table(test.set$category, predictions) # cmat: rows = actual, cols = predicted
cmat[1:5, 1:5]
...
## how did we do predicting each category?
classes <- rownames(cmat)
...
# handle 0/0 cases (class never predicted or never appears)
#precision_k[(TP + FP) == 0] <- NA
#recall_k[(TP + FN) == 0]    <- NA
#f1_k[(is.na(precision_k) | is.na(recall_k) | (precision_k + recall_k) == 0)] <- NA

per_class <- data.frame(
  support = rowSums(cmat),
...
# Weighted by support (class frequencies in actual labels)
w <- per_class$support / sum(per_class$support)
weighted_precision <- sum(w * per_class$precision, na.rm = TRUE)
weighted_recall    <- sum(w * per_class$recall, na.rm = TRUE)
weighted_f1        <- sum(w * per_class$f1, na.rm = TRUE)
