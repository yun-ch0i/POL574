...
## I recommend the most up-to-date version (4.5.2). This will provide more
...
matmul <- mat %*% t(mat) # matrix multiplication
dim(matmul)
...
gsub("not", "NOT", mysentence) # substitute "not" with "NOT" - global substitute
gsub(",", "", mysentence) # works for punctuation

gsub(".", "", mysentence) # some characters are "special" - wildcard that matches any single character
gsub("\\.", "", mysentence) # escape with double backslash
# stringr cheatseet: https://rstudio.github.io/cheatsheets/html/strings.html
...
grep("not", mycleanwords) # indices that CONTAIN not - global regular expression print
...
textplot_wordcloud(reviews.pos.cleandfm, min_count = 5, random_order = F, rotation = 0.25,
                   color = RColorBrewer::brewer.pal(4, "Dark2")) # 8 distink color from Dark 2 palette
