...
blm_tweets <- read.csv("data/blm_samp.csv", stringsAsFactors = F)
...
#   mc.cores = detectCores(), # use all available CPU cores in parallel
#   verbose = TRUE
# )

## Where do these metrics come from? 
## Griffiths 2004 maximizes log-likelihood of the corpus under the model
## Cao et al 2009 minimizes average cosine similarity between topic-word distributions
## Arun 2010 minimize KL divergence between the matrix representing word probabilities for each topic and the topic distribution within the corpus)
## Deveaud 2014 maximizes average Jensen–Shannon distance avg Jensen-Shannon divergence between topics 
...
blm_topics %>% 
  ...
  mutate(log_ratio = log2(topic2 / topic1)) %>% # why log?
  arrange(-abs(log_ratio)) %>%
  slice(c(1:10,(nrow(.)-9):nrow(.))) %>% # the most 10 and least 10 distinctive words
  arrange(-log_ratio) %>%
...
## Combine data
top1 <- data.frame(top_topic = max, date = ymd(blm_tweets_sum$date2)) %>% 
...
blog_stm <- stm(documents=poliblog5k.docs,
                ...
                K=3,
                prevalence = ~rating + s(day), # topics vary by partisanship and time; spline function allows non-linear relationship
...
## topic dynamics E[\theta_{d,k}|day]
plot(prep, 
     "day", 
     blog_stm, 
     topics = c(1,2), 
...
## topic prevalence by party E[\theta_{d,k}|Con] - E[\theta_{d,k}|Lib]
plot(prep, 
     "rating", 
     model = blog_stm,
     method = "difference", 
...
glove <- GlobalVectors$new(rank = DIM, 
                           x_max = 10, # downweights 10+ co-occurences
                           learning_rate = 0.05) 
...
# How does the loss is computed?

## 3.4) Interpreting the output ------------------------------------------------

dim(wv_main) # main vector (word as a target)
word_vectors_context <- glove$components # context vector (word as a neighbor)
...
pretrained <- readRDS("~/Dropbox/POL 574/glove.rds") # GloVe pretrained (https://nlp.stanford.edu/projects/glove/)
dim(pretrained)
