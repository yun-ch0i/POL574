...
pacman::p_load(dplyr, glmnet, quanteda, ROCR, caret, purrr, tidyr)
...
cvout.ridge$lambda.min # best lambda
cvout.ridge$lambda.1se # simpler lambda
cvout.ridge$cvm # mean CV error
cvout.ridge$cvsd # std error

## predict values of test set, given the value of lambda and the 
pred.ridge <- predict(cvout.ridge, s="lambda.min", newx=X.test) # s= value of lambda; 

## how do we know arguments?
## class(model) -> ?predict.<class>
# helpful visualization: https://devopedia.org/bias-variance-trade-off
...
## plot results
plot(pred.tpr, col="red") # plot ROC curve
...
abilities <- as.data.frame(BTabilities(citeModel))
abilities$id <- rownames(abilities)
abilities$lower <- abilities$ability - 1.96 * abilities$s.e.
abilities$upper <- abilities$ability + 1.96 * abilities$s.e.

# sort by ability
abilities <- abilities[order(abilities$ability), ]

dotchart(abilities$ability,
         labels = abilities$item,
         xlim = range(abilities$lower, abilities$upper),
         pch = 19,
         main = "Item abilities with 95% CIs")

# add error bars
for (i in seq_len(nrow(abilities))) {
  lines(c(abilities$lower[i], abilities$upper[i]), c(i, i))
}

# An different data structure
citations_long <- citations.sf %>%
  ...
  mutate(
    winner = if_else(win_type == "win1", journal1, journal2),
    loser = if_else(win_type == "win1", journal2, journal1)
  ) %>%
  uncount(n) %>%
  select(winner, loser)
...
citeModel <- BTm(outcome = 1, winner, loser,
                 refcat = "Biometrika",
                 data = citations_long)
