...
generate <- function(n) {
  ...
  immigration <- rnorm(n, ifelse(party=="D", 0.1, 0.9), 0.5) # rnorm(n, mean, sd)
...
# ids_train <- sample(1:nrow(news_dfm), floor(0.8*n))
ids_train <- createDataPartition(1:nrow(news_dfm), p = 0.8, list = FALSE, times = 1) # preserves class proportions
...
coefs <- svm_mod_linear$finalModel@coef[[1]] # support vector coefficients
mat <- svm_mod_linear$finalModel@xmatrix[[1]] # feature vectors for the support vectors only
dim(mat)

temp <- t(coefs %*% mat)
head(temp[order(temp[,1]),], 10) # most negative words
head(temp[order(-temp[,1]),], 10) # most positive words
...
news_samp <- news_data %>% 
  ...
  group_by(category) %>%
  sample_n(250) %>%  # sample 250 of each to reduce computation time (for lab purposes)
  ungroup() %>%
...
## ASIDE: how would we call an algorithm with ncol(train_x) instead of sqrt(ncol(train_x))? - bagged trees
...
token_importance <- round(importance(rf.base, type = 2), 2) 
# type 1 (mean decrease in accuracy); type 2 (mean decrease in Gini impurity)
