...
################################################# Precept 8: Unsupervised Text Analysis I
...
SOTU_pca$x # document positions (scores) 
View(SOTU_pca$x)  # each observation 

## variable importance for each PC
SOTU_pca$rotation # word weights (loadings)
dim(SOTU_pca$rotation)
colSums(SOTU_pca$rotation^2)
...
nearest.neighbors <- function(query, low_dim_space, N = 5, norm = "l2"){
  cos_sim <- sim2(x = low_dim_space, 
                  y = low_dim_space[query, , drop = FALSE], # drop=F ensures it stays a matrix (not a vector)
                  method = "cosine", 
                  norm = norm)
  nn <- cos_sim <- cos_sim[order(-cos_sim),] # reorder from largest to smallest
...
# apply SVD and project terms/documents into a low-dimensional latent semantic space
...
SOTU_lsa_5_mat <- t(as.textmatrix(SOTU_lsa_5)) # converts the LSA object back into a text matrix
